...
Class Produit{
   ...
   public function add($tab){
    ...
    public function add_cat($id_cat){
        ...
        $query = "INSERT INTO t_cat_prd SET "
                . "id_prd = :id_prd, "
                . "id_cat = :id_cat";
        try {
            $stmt = $this->pdo->prepare($query);
	...
	// Problème 2
    public function del_cat($id_cat){
        try{
            $query = "Delete from t_categorie Where id_cat=:id_cat AND id_prd=:id_prd";
    ...
    function get_all_with_cat(){
        $query = "SELECT PRD.*, GROUP_CONCAT(`nom_cat` SEPARATOR '<br>') AS categories "
                . "FROM t_produits PRD "
    ...
    function get_all_cat(){
        $query = "SELECT CAT.* 
                FROM t_cat_prd CAP 
                    JOIN t_categories CAT ON (CAP.id_cat = CAT.id_cat)
                WHERE id_prd=:id_prd
